...
export class SavannaComponent implements OnInit, OnDestroy {
  fallIntervalId = null;

  topValue = 0;

  topValueString = `${this.topValue}px`;
  ...
  /*   checkImgSrc = (src: string): string => {
    let imgSrc = '';
    const img = new Image();
    img.onload = () => (imgSrc = src);
    img.onerror = () => false;
    img.src = src;
    return imgSrc;
  };
 */
  ngOnInit(): void {
    ...
    this.fallIntervalId = setInterval(() => this.startWordFall(), 500);
  }

  private startWordFall() {
    this.topValue += 10;
    this.topValueString = `${this.topValue}px`;
  }
  ...
  private shuffleWords(words): void {
    ...
    words.forEach((word) => {
      ...
      if (this.wordsForRounds[0].length <= 3) {
      ...
      if (this.wordsForRounds[1].length <= 3) {
      ...
      if (this.wordsForRounds[2].length <= 3) {
      ...
      if (this.wordsForRounds[3].length <= 3) {
        this.wordsForRounds[3].push(word);
      }

      if (!this.wordsForRounds[4]) {
        this.wordsForRounds.push([]);
      }

      if (this.wordsForRounds[4].length <= 3) {
        this.wordsForRounds[4].push(word);
      }
    });

    this.wordsForRounds.forEach((roundWords) => {
      this.answers.push(
        roundWords[Math.floor(Math.random() * roundWords.length)],
      );
    });
    ...
    this.maxRound = this.answers.length;

    console.log('this.wordsForRounds');
    console.log(this.wordsForRounds);
  }
