...
import {IWordPost} from "../../modules/shared/models/wordModels";
...
export class WordsEffect {
  ...
  fetchWords$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() => this.wordService.getWords()
        .pipe(
  ...
  fetchUserWords$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(() => this.wordService.getUserWords()
        .pipe(
  ...
  postUserWord$ = createEffect(() =>
    this.actions$.pipe(
      ofType(postUserWord),
      switchMap(({wordId, word}) => this.wordService.postWord(wordId, word)
        .pipe(
  ...
  putUserWord$ = createEffect(() =>
    this.actions$.pipe(
      ofType(putUserWord),
      switchMap(({wordId, word}) => this.wordService.putWord(wordId, word)
        .pipe(
  ...
  deleteUserWord$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({wordId}) => this.wordService.deleteWord(wordId)
        .pipe(
  ...
  fetchUserAggWords$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({group, filter, wordsPerPage}) => this.wordService.getUserAggWords(group, filter, wordsPerPage)
        .pipe(
  ...
  constructor(private actions$: Actions, private wordService: WordsServiceService) {
  }
